...
import AddCashflowModal from '../components/AddCashflowModal';
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Cashflow</h1>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50">
                <th className="px-6 py-4 text-[13px] font-bold text-slate-700 dark:text-slate-300">Ref</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
              {mockCashflows.map(item => (
                <tr key={item.ref} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                  <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{item.ref}</td>
                  <td className="px-6 py-4 text-sm font-bold text-slate-900 dark:text-white">{item.bank}</td>
                  <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{item.type}</td>
                  <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{item.paymentMethod}</td>
                  <td className={`px-6 py-4 text-sm ${getAmountStyle(item.type)}`}>
                    {item.type.toLowerCase() === 'inflow' ? '+' : '-'}${item.amount.replace(/[-+]/g, '')}
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{item.date}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2 py-1 rounded text-[10px] font-bold border shadow-sm ${getStatusBadgeClass(item.status)}`}>
                      {item.status}
                    </span>
                  </td>
      ...
      {/* Add Cashflow Modal */}
      <AddCashflowModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} />
    </div>
  );
};
