...
import { X, Layers, AlignLeft, DollarSign } from 'lucide-react';
...
const AddAddonModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // POST http://localhost:8000/api/subscriptions/addons/
      const payload = {
        name: formData.name,
      ...
      await api.post('/subscriptions/addons/', payload);
      ...
      console.error(err);
      setError(err.response?.data?.detail || err.response?.data?.error || err.message || 'Failed to create addon');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="relative bg-white dark:bg-[#111624] w-full max-w-md max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Create Addon</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">Add an extra module to subscriptions</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          ...
          <form id="add-addon-form" onSubmit={handleSubmit} className="space-y-5">
            
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Addon Name <span className="text-red-500">*</span></label>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <textarea 
                  ...
                  placeholder="e.g. 50GB additional storage" 
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                />
                <div className="w-11 h-6 bg-red-500 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-teal-300 dark:peer-focus:ring-teal-800 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-slate-600 peer-checked:bg-emerald-500"></div>
