...
import { X, Building2, User, AlignLeft, ToggleLeft } from 'lucide-react';
...
const AddDepartmentModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // POST http://localhost:8000/api/settings/departments/
      const payload = {
        ...formData,
        // Convert to integer since department_head is an ID in the API
        department_head: formData.department_head ? parseInt(formData.department_head, 10) : null
      };

      await api.post('/settings/departments/', payload);
      setSuccess('Department created successfully!');
      setTimeout(() => {
        onClose();
        // Reset form or trigger a refresh in parent here if needed
      }, 1500);
    } catch (err) {
      console.error(err);
      setError(err.response?.data?.detail || err.response?.data?.error || err.message || 'Failed to create department');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-[#111624] w-full max-w-lg max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          ...
          <form id="add-department-form" onSubmit={handleSubmit} className="space-y-5">
            
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Department Name <span className="text-red-500">*</span></label>
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Department Head ID <span className="text-red-500">*</span></label>
              <div className="relative">
