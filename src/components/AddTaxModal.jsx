...
import { X, Receipt, Activity, Tag, Settings } from 'lucide-react';
...
const AddTaxModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/finance/taxes/', formData);
      ...
      console.error(err);
      setError(err.response?.data?.detail || err.response?.data?.error || err.message || 'Failed to create tax');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="relative bg-white dark:bg-[#111624] w-full max-w-md rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 shrink-0 bg-slate-50 dark:bg-slate-900/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">Add New Tax</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">Configure a new tax rate</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          ...
          <form id="add-tax-form" onSubmit={handleSubmit} className="space-y-5">
            
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Tax Name <span className="text-red-500">*</span></label>
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Rate (%) <span className="text-red-500">*</span></label>
              <div className="relative group">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500 font-bold transition-colors">%</span>
                <input 
                  type="number" 
                  step="0.01"
                  name="rate" 
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Applied To <span className="text-red-500">*</span></label>
              <div className="relative group">
                <Settings className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500 transition-colors" size={16} />
                <select 
                  name="applied_to" 
                  required 
