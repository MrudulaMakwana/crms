...
import { X, User, Building, CreditCard, DollarSign, Calendar, Activity } from 'lucide-react';
...
const AddPaymentModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.detail || err.response?.data?.error || err.message || 'Failed to record payment');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="relative bg-white dark:bg-[#111624] w-full max-w-lg rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 shrink-0 bg-slate-50 dark:bg-slate-900/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">Add New Payment</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">Record an outgoing payment</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          ...
          <form id="add-payment-form" onSubmit={handleSubmit} className="space-y-5">
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Payee <span className="text-red-500">*</span></label>
