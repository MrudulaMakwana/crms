...
import { X, Calendar as CalendarIcon, AlignLeft, MapPin, Users, ToggleLeft, Hash } from 'lucide-react';
...
const AddEventModal = ({ isOpen, onClose }) => {
  const [formData, setFormData] = useState({
    ...
    is_all_day: false,
    location: '',
    attendees: '' // will be comma separated string in UI
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // POST http://localhost:8000/api/calendar/events/
      // Parse attendees string to array of numbers
      const parsedAttendees = formData.attendees
        .split(',')
      ...
      console.error(err);
      setError(err.response?.data?.detail || err.response?.data?.error || err.message || 'Failed to create event');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="relative bg-white dark:bg-[#111624] w-full max-w-lg max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Schedule Event</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">Create a new meeting or event</p>
            </div>
          </div>
          <button 
            onClick={onClose}
          ...
          <form id="add-event-form" onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-center gap-3">
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                <span className="ml-3 text-sm font-medium text-slate-700 dark:text-slate-300">All Day Event</span>
              </label>
            </div>
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Attendees (IDs)</label>
              <div className="relative">
              ...
              <p className="text-xs text-slate-500">Comma-separated list of employee IDs</p>
            </div>
